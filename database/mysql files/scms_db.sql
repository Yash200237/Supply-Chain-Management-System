...
CREATE TABLE `DriverAssistant` (
  ...
  CHECK (
    `weekly_hours` BETWEEN 0 AND 60
  ) -- Constraint for weekly_hours
);
...
CREATE TABLE `Truck` (
  ...
  -- Updated to NOT NULL and VARCHAR(10)
  `capacity` NUMERIC(8, 2),
  PRIMARY KEY (`truck_Id`),
  CONSTRAINT `fk_truck_store` FOREIGN KEY (`store_ID`) REFERENCES `Store`(`store_ID`) ON DELETE
  SET NULL
);
...
CREATE TABLE `Order` (
  ...
  CONSTRAINT `fk_order_schedule` FOREIGN KEY (`schedule_ID`) REFERENCES `TruckSchedule`(`schedule_ID`) ON DELETE
