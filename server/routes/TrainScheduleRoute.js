...
router.get("/api/TrainSchedule", (req, res) => {
  const sql =
    "SELECT train_ID, day, time, capacity,remainingCapacity ,destination FROM Train";
...
router.post("/api/AssignOrders", (req, res) => {
  ...
  con.query(checkPendingOrdersSql, [city], (err, result) => {
    ...
    con.query(fetchPendingOrdersSql, [city], (err, orders) => {
      ...
      con.query(
        fetchTrainRemainingCapacitySql,
        [train_ID],
        (err, trainResult) => {
          if (err) {
          ...
          if (
            trainDepartureDateTime - currentDateTime <
            twentyFourHoursInMillis
          ) {
            return res.status(400).json({
              error:
                "Cannot assign orders within 24 hours before train departure.",
            });
          }
        ...
        `;
          ...
          const trainScheduleValues = assignedOrders.map((order_id) => [
            train_ID,
            order_id,
            manager_ID,
            departureDate,
          ...
          `;
            ...
            `;
              ...
              con.query(
                updateRemainingCapacitySql,
                [remainingCapacity, train_ID],
                (err) => {
                  if (err) {
